...
Integrarea Paynet este server-side prin `server.js`, ca datele merchant si parola API sa nu ajunga
in browser.

1. Copiaza `.env.example` in `.env`.
2. Completeaza `PAYNET_MERCHANT_PASSWORD` cu parola API primita de la Paynet.
3. Porneste site-ul cu `npm start`.
4. Deschide `http://localhost:8080`.

Fluxul este:

- `POST /api/paynet/checkout` primeste rezervarea.
- Serverul cere token la `POST /auth`.
